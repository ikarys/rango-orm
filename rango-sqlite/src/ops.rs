...
where
    ...
    M: Model + ModelValues,
{
    let sql = format!("DELETE FROM \"{}\" WHERE \"{}\" = ?", M::table_name(), M::pk_column());
    ...
    M: Model + ModelValues + FromRow,
{
    ...
    if let Some(r) = row {
        return Ok((M::from_row(&SqliteRangoRow(r)).map_err(|e| anyhow::anyhow!("{}", e))?, true));
    }
    ...
    Ok((M::from_row(&SqliteRangoRow(row)).map_err(|e| anyhow::anyhow!("{}", e))?, false))
}
    ...
    M: FromRow,
{
    ...
    let rows = q.fetch_all(executor).await.map_err(|e| anyhow::anyhow!("{}", e))?;
    rows.into_iter()
        .map(|r| M::from_row(&SqliteRangoRow(r)).map_err(|e| anyhow::anyhow!("{}", e)))
        .collect()
}
...
fn build_update_sql<M: Model + ModelValues>(model: &M) -> (String, Vec<SqlValue>) {
    ...
    let sql = format!(
        ...
        M::table_name(), set_clauses.join(", "), pk_col
    );
...
fn build_lookup_sql<M: Model + ModelValues>(
    ...
    let conditions: Vec<String> = lookup.iter()
        .map(|(col, _)| format!("\"{}\" = ?", col))
        .collect();
    let sql = format!(
        "SELECT * FROM \"{}\" WHERE {} LIMIT 1",
